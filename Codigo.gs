      ...
      respSheet.getRange(numResponses,numColumns).setValue(f);
        ...
        for (i = 0; i < numResponses; i++) {
          if (respSheet.getRange(numResponses-i,numColumns-2).getValue()=="") {
            respSheet.getRange(numResponses-i,numColumns-2).setValue(Utilities.formatDate(new Date(), "GMT-3", "dd/MM/yyyy HH:mm:ss")).setFontColor("red");
          ...
          //fill data for first respondent
              if (numResponses==2) {
                respSheet.getRange(numResponses,numColumns+1).setValue(Utilities.formatDate(new Date(), "GMT-3", "dd/MM/yyyy HH:mm:ss"));
                ...
                respSheet.getRange(numResponses,numColumns+3).setValue(f);
                  ...
                  respSheet.getRange(numResponses,numColumns+1).setValue(Utilities.formatDate(new Date(), "GMT-3", "dd/MM/yyyy HH:mm:ss")).setFontColor("red");
